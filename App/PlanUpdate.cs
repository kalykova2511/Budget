...
{
    public partial class PlanUpdate : Form
    {
        int id;
        int type;
        MainForm f;
        
        public PlanUpdate(int id, int type, MainForm f)
        {
            InitializeComponent();
            this.id = id;
            this.type = type;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (type == 1) //Доходы
            {
                using (var co = new MySqlConnection("server=christlh.beget.tech;port=3306;database=christlh_fin_sys;user=christlh_fin_sys;password=OisbkfA5oMaX;"))
                {
                    MySqlCommand insert = new MySqlCommand("UPDATE future_incomes SET " +
                    "bill = @bill, " +
                    "start_date = @sd, " +
                    "end_date = @ed, " +
                    "category_id = @cid " +
            ...
            else //Расходы
            {
                using (var co = new MySqlConnection("server=christlh.beget.tech;port=3306;database=christlh_fin_sys;user=christlh_fin_sys;password=OisbkfA5oMaX;"))
                {
                    MySqlCommand insert = new MySqlCommand("UPDATE future_expenses SET " +
                    "bill = @bill, " +
                    "start_date = @sd, " +
                    "end_date = @ed, " +
                    "category_id = @cid " +
